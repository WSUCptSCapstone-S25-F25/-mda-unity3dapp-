            ...
            foreach (var comp in materialComponents)
            {
                ...
                if (renderer != null)
                {
                    ...
                    if (isOn)
                    {
                        ...
                        // Ensure a BoxCollider as a trigger for detection
                        BoxCollider boxTrigger = comp.GetComponent<BoxCollider>();
                        if (!boxTrigger)
                            boxTrigger = comp.AddComponent<BoxCollider>();

                        boxTrigger.isTrigger = true;
                        boxTrigger.enabled = true;

                        // Add Detector
                        if (!comp.GetComponent<ConductiveCollisionDetector>())
                            comp.AddComponent<ConductiveCollisionDetector>();
                    }
                    else
                    {
                        ...
                        // Remove or disable only the BoxCollider added for detection
                        BoxCollider boxTrigger = comp.GetComponent<BoxCollider>();
                        if (boxTrigger)
                            boxTrigger.enabled = false;

                        // Remove collision detector 
                        var detector = comp.GetComponent<ConductiveCollisionDetector>();
                        if (detector)
                            Destroy(detector);
                    }
                }
